...
function parseLines(text: string) {
  ...
  for (const raw of lines) {
    ...
    if (parts.length >= 2) {
      ...
      // Examples: "...CA 94111 Stacker+1", "...CO 80202 Wikipedia+1"
      
      // First, remove anything after the last occurrence of a state zip code pattern
      const zipMatch = address.match(/(.*\b(?:CA|CO|California|Colorado)\s+\d{5}(?:-\d{4})?)/);
      if (zipMatch) {
...
function isRelaxType(types: string[] = []): boolean {
  const set = new Set(types);
  return (
    set.has('spa') ||
    set.has('beauty_salon') ||
    set.has('hair_care') ||
    set.has('massage') ||
    set.has('sauna')
  );
...
function isFitnessType(types: string[] = []): boolean {
  const set = new Set(types);
  return (
    set.has('yoga_studio') ||
    set.has('fitness_center') ||
    set.has('gym') ||
    set.has('pilates_studio') ||
    set.has('sports_club')
  );
}

function isRoadtripType(types: string[] = []): boolean {
  const set = new Set(types);
  return (
    set.has('natural_feature') ||
    set.has('park') ||
    set.has('campground') ||
    set.has('rv_park') ||
...
function matchesRelaxKeywords(name: string): boolean {
  const n = name.toLowerCase();
  return /spa|sauna|massage|bodywork|wellness|banya|hammam/.test(n);
}

function matchesFitnessKeywords(name: string): boolean {
  const n = name.toLowerCase();
  return /yoga|pilates|fitness|gym|crossfit|cycle|barre|boxing|martial|studio|class/.test(n);
}

function categoryFromTypes(types: string[] = []): string | undefined {
  if (isRelaxType(types)) return 'relax-recharge';
  if (isFitnessType(types)) return 'fitness-classes';
  if (isRoadtripType(types)) return 'road-trip-getaways';
  const set = new Set(types);
...
async function main() {
  const argv = ARGV;
  if (argv.length === 0) {
    console.log('Usage: tsx scripts/import-denver-from-pdf.ts "Denver Locations.pdf" [--city "Denver, CO"] [--dry] [--limit 200]');
    process.exit(1);
  }

  const pdfPath = path.resolve(argv[0]);
  // Support --city "..." or --city '...' or --city VALUE
  const cityRegex = /--city\s+\"([^\"]+)\"|--city\s+\'([^\']+)\'|--city\s+([^\s]+)/;
  const cityArg = argv.join(' ').match(cityRegex);
  const CITY = (cityArg?.[1] || cityArg?.[2] || cityArg?.[3] || 'Denver, CO').trim();
  const DRY = DRY_FLAG;
  const limitMatch = argv.join(' ').match(/--limit\s+(\d+)/);
  const LIMIT = limitMatch ? parseInt(limitMatch[1], 10) : 300;
  ...
  console.log(`Parsed ${rows.length} rows from PDF. City bias: ${CITY}${DRY ? ' [dry-run]' : ''}`);
  ...
  for (const row of rows.slice(0, LIMIT)) {
    ...
    try {
      ...
      if (DRY) {
        console.log(`[dry] ${listing.title} → ${listing.category}`);
      } else {
        const ok = await upsertListing(listing, photos);
        if (ok) console.log(`✓ Upserted: ${listing.title}`);
      }
